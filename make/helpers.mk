...
_full_clean = { $(_kill_zombies); $(DOCKER_COMPOSE) down -v --remove-orphans; \
	docker run --rm -v "$$(pwd)/volume:/vol" -v "$$(pwd)/$(OUTPUT_DIR):/tmp_clean" \
		alpine sh -c "rm -rf /vol/* /tmp_clean/*"; } > logs/run_clean_full.log 2>&1

_light_clean = { $(DOCKER_COMPOSE) down --remove-orphans; \
	docker run --rm -v "$$(pwd)/$(OUTPUT_DIR):/tmp_clean" \
		alpine sh -c "rm -rf /tmp_clean/*"; } > logs/run_clean_light.log 2>&1
