...
def ejecutar_query(nombre_dataset):
    ...
    for chunk in pd.read_csv(ruta_datasets / nombre_dataset, chunksize=chunksize):
        transacciones_usd = chunk[chunk['Payment Currency'] == "US Dollar"]
        monto_limite = 50  
        ...
        if not resultado.empty:
            if first_chunk:
                resultado.to_csv(solucion_path, mode='w', index=False)
                first_chunk = False
            else:
                resultado.to_csv(solucion_path, mode='a', header=False, index=False)

    if first_chunk:
        resultado = pd.DataFrame(columns=['From Bank', 'Account', 'To Bank', 'Account.1', 'Amount Paid'])
