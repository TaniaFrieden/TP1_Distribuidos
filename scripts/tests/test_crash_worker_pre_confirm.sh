...
echo "=== [crash-worker-pre-confirm] Target: $TARGET, $CANT_CLIENTES cliente(s) ==="
make down
timeout 10s docker run --rm -v "$(pwd)/volume:/cleanup" alpine sh -c "rm -rf /cleanup/*" 2>/dev/null \
    || rm -rf volume/* 2>/dev/null || true

echo "=== Levantando sistema con ${TARGET}_CRASH=CRASH_BEFORE_FINISHED_CONFIRMATION ==="
eval "${TARGET}_CRASH=CRASH_BEFORE_FINISHED_CONFIRMATION make start"
